...
class GalleryPreload
{
	...
	public static var avaiableCharacters:Array<String> = ['Bf', 'Pico'];
	...
	public static function preloadImages(galleryDirectory:String):Void
	{
		Thread.create(() ->
		{
			...
			{
				trace('Gallery directory not found or empty: $galleryPath');
				return;
			}

			totalImageItems = imagesOnFolder.length;

			for (num in 0...imagesOnFolder.length)
			{
				var image = imagesOnFolder[num];
