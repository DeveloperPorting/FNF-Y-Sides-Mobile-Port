...
class NewGalleryState extends MusicBeatState
{
	...
	static var musicSongsArray:Array<Dynamic> = [
		// Song name - Is pico mix?
		['tutorial', false],
	];

	var optionsGrp:FlxTypedGroup<GalleryObject>;
	var optionsArr:Array<String> = ['outdated_concepts', 'music', 'bored'];
	...
	function handleInputs()
	{
		if (isPreviewingImages || isPreviewingMusics)
		{
			...
			if (controls.BACK)
			{
				...
				FlxTween.tween(infoButton, {alpha: 0}, tweenTransSpeed, {
	...
	{
		if (playMusicTimer > 1 || firstTime)
			FlxG.sound.playMusic(Paths.sound('gallery/whiteNoise')); // behaviour as music so it loops
	...
	function playMusic()
	{
		...
		if (Assets.exists('assets/songs/${musicSongsArray[curSelectedMusics][0]}/Inst-$charSuffix.ogg'))
		{
			if (FlxG.sound.music != null)
	...
	function generateImages(folderName:String)
	{
		...
		for (num => image in imagesOnFolder)
		{
			...
			try
			{
				...
				var imageData = haxe.Json.parse(content);
			...
			spr.setGraphicSize(Std.int((spr.width * 400) / spr.height), 400);
			spr.screenCenter();
			spr.x = 800;
			spr.x += -(spr.width / 2);
		...
		FlxTween.tween(infoButton, {alpha: 1}, tweenTransSpeed, {
			ease: FlxEase.quartInOut,
			onComplete: function(twn:FlxTween)
			{
	...
	function lockedWeeks(unlockEverything:Bool = false)
	{
		...
		if (NewStoryMenuState.weekCompleted.exists('week1'))
		{
			musicSongsArray.push(['bopeebo', false]);
			musicSongsArray.push(['fresh', false]);
			if (picoUnlocked() && BeatenSongs.isSongBeaten('fresh-pico'))
				musicSongsArray.push(['fresh', true]);
			musicSongsArray.push(['dad-battle', false]);
		}
		...
		if (NewStoryMenuState.weekCompleted.exists('week2'))
		{
			musicSongsArray.push(['spookeez', false]);
			musicSongsArray.push(['south', false]);
			if (picoUnlocked() && BeatenSongs.isSongBeaten('south-pico'))
				musicSongsArray.push(['south', true]);
			musicSongsArray.push(['monster', false]);
		}
		...
		{
			musicSongsArray.push(['pico', false]);
			if (picoUnlocked() && BeatenSongs.isSongBeaten('pico-pico'))
				musicSongsArray.push(['pico', true]);
			musicSongsArray.push(['philly-nice', false]);
			musicSongsArray.push(['blammed', false]);
		}
		...
		if (NewStoryMenuState.weekCompleted.exists('week4'))
		{
			musicSongsArray.push(['satin-panties', false]);
			musicSongsArray.push(['high', false]);
			if (picoUnlocked() && BeatenSongs.isSongBeaten('high-pico'))
				musicSongsArray.push(['high', true]);
			musicSongsArray.push(['milf', false]);
			if (picoUnlocked() && BeatenSongs.isSongBeaten('milf-pico'))
				musicSongsArray.push(['milf', true]);
		}
